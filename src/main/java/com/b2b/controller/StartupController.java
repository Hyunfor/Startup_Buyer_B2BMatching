...
public class StartupController {
	...
	public String remove(@RequestParam("startup_id") String startup_id, HttpSession session,
			@ModelAttribute("cri") SearchCriteria cri, RedirectAttributes rttr) throws Exception {

		// 삭제하려면 로그인한 정보와 게시글의 작성자가 일치

		// 1)로그인 정보 가져오기
		StartUpUserVO user = (StartUpUserVO) session.getAttribute("login");
		...
		// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		if (user.getS_id().equals(vo.getStartup_id())) {
			// 정보일치 -> 게시글 삭제
	...
	public void readPage(@RequestParam("startup_id") String startup_id, @ModelAttribute("cri") SearchCriteria cri,
